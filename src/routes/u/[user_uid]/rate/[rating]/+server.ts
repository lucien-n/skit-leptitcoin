...
export const GET = async ({ params, locals: { supabase, getSession } }) => {
  ...
  const {
    data: [{ uid: userUid }],
  } = await supabase.from("profiles").select("uid").eq("uid", ratedUserUid);
  if (userUid !== ratedUserUid) throw error(404, { message: "user-not-found" });

  try {
    const { status, statusText } = await supabase
      .from("ratings")
      .upsert({ rated: ratedUserUid, rater: user.id, value: rating });
    console.log(status === 201 ? "Successfuly rated" : statusText);
  } catch (e) {
    console.warn(e);
